import { Check, MoreHorizontal, SquareArrowOutUpRight } from "lucide-react";
import { useState } from "react";
import { toast } from "sonner";
import SiteSketchEditor from "@/components/SiteSketchEditor";
import type { GeneratedSection, ProjectState } from "./model";
...
export function WorkspacePreview({ project, editorData }: { project: ProjectState; editorData: Record<string, unknown> }) {
  ...
  return <div className="space-y-6 pb-12"><div className="page-heading"><div><div className="eyebrow text-indigo-500">Step 3 of 3 · Preview</div><h1 className="mt-2 font-display text-[32px] font-semibold tracking-[-0.04em] text-slate-900 sm:text-[40px]">Take a look at your site.</h1><p className="mt-3 max-w-[500px] text-sm leading-relaxed text-slate-500">Review your working draft before you keep editing. This preview stays inside SiteSketch—it is not a published website.</p></div><div className="heading-actions"><span className="status-pill status-pill-light"><span className="live-dot" />{activePage ? "Live draft" : "Plan draft"}</span>  <button className="button-dark" onClick={sharePreview}><SquareArrowOutUpRight size={14} />Copy preview link</button></div></div><div className="preview-window"><div className="preview-browser"><div className="browser-dots"><i /><i /><i /></div><span className="browser-url">preview.sitesketch.local / {project.projectName.toLowerCase().replace(/ /g, "-")}</span><div className="flex items-center gap-2 text-slate-400"><span>↻</span><MoreHorizontal size={15} /></div></div>{pages.length > 0 && <div className="flex items-center gap-2 border-b border-slate-200 bg-white px-4 py-3">{pages.map(page => <button key={page.id} onClick={() => setActivePageId(page.id)} className={`rounded-full px-3 py-1.5 text-[11px] font-semibold ${page.id === activePage?.id ? "bg-slate-900 text-white" : "text-slate-500 hover:bg-slate-100"}`}>{page.name}</button>)}</div>}{activePage ? <iframe title={`${project.projectName} ${activePage.name} preview`} sandbox="" srcDoc={createPreviewDocument(activePage)} className="h-[680px] w-full border-0 bg-white" /> : <div className="flex min-h-[520px] items-center justify-center bg-[#161d38] p-10 text-center text-white"><div><div className="preview-check mx-auto"><Check size={13} /></div><h2 className="mt-4 font-display text-3xl">Your plan is ready to build.</h2><p className="mt-3 max-w-md text-sm text-slate-300">Use Edit from the mode navigation to create the first saved preview for this project.</p></div></div>}</div><div className="preview-bottom"><div className="flex items-center gap-3"><div className="preview-check"><Check size={13} /></div><div><div className="text-xs font-semibold text-slate-700">{activePage ? "Saved editor output is ready to refine" : "Your site is ready to build"}</div><div className="mt-0.5 text-[11px] text-slate-400">{activePage ? `${pages.length} saved page${pages.length === 1 ? "" : "s"} rendered from the builder.` : `${project.sections.length} planned sections are ready for the builder.`}</div></div></div><span className="text-[11px] font-medium text-slate-500">Use the mode navigation to continue editing.</span></div></div>;
}
