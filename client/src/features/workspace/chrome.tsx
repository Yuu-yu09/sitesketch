...
export function WorkspaceTopbar({ mode, setMode, project, onSave, user, onLogout, projects, onNew, onOpenProject, onRenameProject, onDeleteProject }: WorkspaceTopbarProps) {
  ...
  return <header className="topbar relative sticky top-0 z-20 border-b border-[#e4e6eb] bg-[#f8f9fb]/95 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
    <div className="flex min-h-[76px] items-center justify-between gap-3">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="relative hidden min-w-0 sm:block">
          <button type="button" onClick={openProjects} className="project-switcher-trigger" aria-expanded={projectsOpen} aria-haspopup="menu"><FolderOpen size={13} />My projects<ChevronDown size={13} /></button>
      ...
      <button type="button" className="icon-button workspace-menu-toggle lg:hidden" onClick={() => setMobileOpen(value => !value)} aria-label={mobileOpen ? "Close workspace menu" : "Open workspace menu"} aria-expanded={mobileOpen} aria-controls="workspace-mobile-menu">{mobileOpen ? <X size={18} /> : <Menu size={19} />}</button>
    ...
    {mobileOpen && <div id="workspace-mobile-menu" className="mobile-workspace-menu" role="menu" aria-label="Workspace navigation">
