...
import { WorkspaceTopbar } from "@/features/workspace/chrome";
...
function PlanView({ project, setProject, onGenerate, isGenerating, onRenameProject, onDeleteProject }: { project: ProjectState; setProject: (project: ProjectState) => void; onGenerate: () => void; isGenerating: boolean; onRenameProject: () => void; onDeleteProject: () => void }) {
  ...
  return <div className="space-y-6 pb-12"><div className="page-heading"><div><div className="eyebrow text-indigo-500">Step 1 of 3 · Plan</div><h1 className="mt-2 font-display text-[32px] font-semibold tracking-[-0.04em] text-slate-900 sm:text-[40px]">Let’s make your idea clear.</h1><p className="mt-3 max-w-[540px] text-sm leading-relaxed text-slate-500">Choose what your site needs to say. You can change anything later—there’s no wrong place to start.</p></div></div>
    <div className="grid gap-5 xl:grid-cols-[1.06fr_1.42fr_1fr]">
      <div className="space-y-5"><div className="card project-summary p-5"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="eyebrow text-slate-400">Your project</div><h2 className="mt-2 truncate font-display text-[23px] font-semibold text-slate-800">{project.projectName}</h2><p className="mt-1 text-xs text-slate-400">{project.projectType} <span className="mx-1 text-slate-300">·</span> Draft</p></div><div className="project-actions" aria-label="Project actions"><button onClick={onRenameProject} className="project-action-button" title="Rename project" aria-label="Rename project"><MoreHorizontal size={17} /></button><button onClick={onDeleteProject} className="project-action-button project-action-danger" title="Delete project" aria-label="Delete project"><Trash2 size={15} /></button></div></div><div className="mt-5 grid gap-3"><label className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">What kind of site is this?<select value={project.projectType} onChange={(event) => updateProjectType(event.target.value)} className="select-field mt-2 w-full">{projectTypeOptions.map(option => <option key={option}>{option}</option>)}</select></label><label className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">What should it help you do?<select value={project.purpose} onChange={(event) => setProject({ ...project, purpose: event.target.value })} className="select-field mt-2 w-full">{purposeOptions.map(option => <option key={option}>{option}</option>)}</select></label></div><div className="mt-5 rounded-xl bg-[#f5f6f8] p-3.5"><div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400"><Gauge size={13} /> Your goal</div><p className="mt-2 text-[12px] font-medium leading-relaxed text-slate-600">{project.purpose}</p></div></div>
        <div className="ai-card"><div className="ai-glow" /><div className="relative"><div className="flex items-center gap-2 text-indigo-100"><div className="ai-icon"><WandSparkles size={14} /></div><span className="eyebrow text-indigo-200">Optional starting point</span></div><h3 className="mt-4 font-display text-[19px] font-semibold leading-tight text-white">Tell us what you’re making.</h3><p className="mt-2 text-[11px] leading-relaxed text-indigo-100/65">Use everyday words. We’ll suggest a simple structure that you can edit yourself.</p><textarea aria-label="Describe your site" value={project.prompt} onChange={(e) => setProject({ ...project, prompt: e.target.value })} className="ai-textarea mt-4" rows={3} placeholder="For example: a friendly site for my local bakery" /><button onClick={onGenerate} disabled={!project.prompt.trim() || project.prompt.trim().length < 10 || isGenerating} className="button-ai mt-3 w-full justify-center disabled:cursor-not-allowed disabled:opacity-60"><Sparkles size={14} />{isGenerating ? "Making your outline…" : "Suggest an outline"}</button></div></div></div>
...
export default function Home() {
  ...
  const saveState = createProject.isPending || updateProject.isPending || saveEditor.isPending ? "saving" : "saved";
